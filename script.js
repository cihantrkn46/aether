...
class WaveVisualizer {
    ...
    position() {
        ...
        this.canvas.style.left = `${rect.left + rect.width / 2 + scrollX}px`;
    ...
    draw() {
        ...
        for (let i = 0; i < barCount; i++) {
            ...
            const r = barWidth / 2;
            this.ctx.beginPath();
            // roundRect polyfill olmadan çalışabilmesi için fallback
...
class CameraOCR {
    constructor(onText) {
        ...
        this.FRAME_DIFF_THRESHOLD = 0.08;
    }
    ...
    frameDiffPercent(prevHash, currHash) {
        ...
        return diff / (prevHash.length * 64);
    }
...
class App {
    ...
    async performTranslation(force = false, retryCount = 0) {
        // iOS'ta keyboard açıkken scroll pozisyonunu koru
        const scrollPos = window.scrollY || window.pageYOffset || 0;
        const activeEl = document.activeElement;
        const isKeyboardOpen = activeEl && activeEl.tagName === 'TEXTAREA';
    ...
    startListening(which) {
        ...
        if (this.visualizer.audioCtx && this.visualizer.audioCtx.state === 'suspended') {
            this.visualizer.audioCtx.resume().catch(() => {});
        }
        if (!this.SpeechRecognitionImpl) {
    ...
    bindEvents() {
        ...
        document.querySelectorAll('.content, .panel-body').forEach(el => {
            el.addEventListener('touchstart', () => {}, passiveOptions);
            el.addEventListener('touchmove', () => {}, passiveOptions);
        });
...
window.addEventListener('beforeunload', () => {
    ...
    appInstance.visualizer?.stop();
    if (appInstance._resizeHandler) {
        window.removeEventListener('resize', appInstance._resizeHandler);
    }
    if ('visualViewport' in window) {
